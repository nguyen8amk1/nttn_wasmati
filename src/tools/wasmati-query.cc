...
// NOTE: @Nguyen: These are the input files (if they are inputed, their values are here)
...
int ProgramMain(int argc, char** argv) {
    ...
    // NOTE: @Nguyen: ParseOptions will read the options then modifies s_<filename> stuff. 
    ParseOptions(argc, argv);
    Graph* graph = nullptr; // NOTE: @Nguyen: graph = CPG 
    ...
    Query::setGraph(graph); // NOTE: @Nguyen: Query input = CPG + query code (4 forms ??)

    // Evaluate file if provided
    
    // NOTE: @Nguyen: this is where so the WQL is loaded 
    ...
    // NOTE: @Nguyen: This is the native mode -> CPG ?? Is this what we want ?? 
    if (native) {
        auto startVulns = std::chrono::high_resolution_clock::now();
        std::list<Vulnerability> vulns;

       
        // NOTE: @Nguyen this part is important 
        // Works by macro 
        // Let's try to enable to trigger the Buffer Over cases 
        // Let's see it's input and outputs @Current 
        // ...
        // 
        // 

        // NOTE: @Nguyen: Need to write more of these ?? 
